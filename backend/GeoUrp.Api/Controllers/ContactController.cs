...
public sealed class ContactController : ControllerBase
{
    ...
    public ActionResult<ApiResponse<ContactMessage>> Create(
        [FromBody] ContactMessage request,
        [FromServices] InMemoryStore store)
    {
        request.Id = store.ContactMessages.Count == 0 ? 1 : store.ContactMessages.Max(x => x.Id) + 1;
    ...
    public ActionResult<ApiResponse<IEnumerable<ContactMessage>>> GetAll([FromServices] InMemoryStore store)
        => Ok(ApiResponse<IEnumerable<ContactMessage>>.Ok(store.ContactMessages.OrderByDescending(x => x.CreatedAt)));
}
